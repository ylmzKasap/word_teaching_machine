...
export const PageItem = (props) => {
    ...
    const { 
        username, draggedElement, setDraggedElement, directory,
        setReRender, directoryInfo, columnNumber,
        isDragging, categoryDrag, cloneTimeout, resetDrag,
        items, setRequestError, rootDirectory } = useContext(ProfileContext);
    ...
    const handleMouseDown = (event) => {
        ...
        if (props.type === 'category' || targetElem.className === 'category-description') {           
            if (!['category-header', 'category-description'].includes(targetElem.className)) {
                return
            }
        }
    ...
    const handleMouseUp = (event) => {
        ...
        if (targetElem.className === "file" && !isDragging) {
            ...
            {state: {allPaths: props.words, directory: directory, rootDirectory: rootDirectory}});
        } 

        if (['file', 'category', 'thematic-folder'].includes(targetElem.className)
    ...
    const handleHover = (event) => {
        ...
        if (event.type === 'mouseover') {
            setItemStyle({"boxShadow": "rgb(211, 210, 210) 0px 0px 5px 3px", "backgroundColor": props.color});
    ...
    return (
        <div className={
            ...
            style={selfStyle} draggable="false">

            {(
                (props.type === ('category') && draggedElement.type === 'category' && categoryDrag
                    && !cloneTimeout.exists && draggedElement.id !== props.id)
                || props.type !== 'category') &&
                    <Filler 
            ...
            <ItemContext.Provider value={
                ...
                {props.type !== 'category' ?
                    <ItemContainer /> : <CategoryContainer />
                }
            </ItemContext.Provider>

            {((items.length === props.order && directoryInfo.item_type !== 'thematic_folder')
...
const ItemContainer = () => {
    const { parentProps, itemStyle, folderStyle, handleDoubleClick,
    handleMouseDown, handleMouseUp, handleHover} = useContext(ItemContext);
    ...
    const createThumbnail = () => {
        ...
        for (let i = 0; i < parentProps.words.length; i++) {
            ...
            if (thumbnail.length >= 4) {
                ...
                thumbnail.push(
                    <img className={`deck-thumbnail${parentProps.words.length < 4 ? ' n-1' : ''}`}
                    src={`media\\${wordPath}`} key={`${wordStem}-${i}`}
                    alt={`${wordStem}-${i}`} draggable='false'/>)
            }
            if (parentProps.words.length < 4) { break };
        }
        return thumbnail;
    }
...
const CategoryContainer = () => {
    ...
    const { setDeckDisplay, isDragging, setCategoryId } = useContext(ProfileContext);

    const { parentProps, itemStyle,
        handleMouseDown, handleMouseUp, handleHover} = useContext(ItemContext);
